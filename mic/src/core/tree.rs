...
mod tests {
    ...
    fn tree_diff_detects_changes() {
        ...
        old.root_mut().insert(TreeEntry::file("a.txt", hash1, 100, 0o644));
        old.root_mut().insert(TreeEntry::file("b.txt", hash2, 200, 0o644));

        let mut new = Tree::new();
        new.root_mut().insert(TreeEntry::file("a.txt", hash1, 100, 0o644)); // unchanged
        new.root_mut().insert(TreeEntry::file("b.txt", hash3, 200, 0o644)); // modified
        new.root_mut().insert(TreeEntry::file("c.txt", hash2, 300, 0o644)); // added
