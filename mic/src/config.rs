...
use crate::constants::{
    is_first_party_url, FIRST_PARTY_CLIENT_ID, TOKEN_REFRESH_BUFFER_SECS,
};
...
impl Config {
    ...
    pub fn set_alias(&mut self, alias: &str, project_ref: &str) {
        self.aliases.insert(alias.to_string(), project_ref.to_string());
    }
    ...
    pub fn resolve_project<'a>(&'a self, ref_str: &'a str) -> &'a str {
        self.aliases.get(ref_str).map(|s| s.as_str()).unwrap_or(ref_str)
    }
...
pub fn config_dir() -> Result<PathBuf> {
    if let Ok(mic_home) = std::env::var("MIC_HOME") {
...
pub fn store_tokens(tokens: &StoredTokens) -> Result<()> {
    ...
    #[cfg(unix)]
    {
        ...
        fs::set_permissions(&path, fs::Permissions::from_mode(0o600))
            .map_err(|e| MicError::ConfigError(format!("Failed to set token permissions: {}", e)))?;
    }
...
async fn refresh_tokens(grpc_url: &str, refresh_token: &str) -> Result<StoredTokens> {
    ...
    let client_id = if let Some(ref id) = server_config.client_id {
        ...
        return Err(MicError::ConfigError("No client_id configured for server".to_string()));
    };
