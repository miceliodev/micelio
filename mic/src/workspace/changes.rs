...
use crate::workspace::{WorkspaceManifest, is_safe_path};
...
pub fn collect_changes(workspace_root: &Path, manifest: &WorkspaceManifest) -> Result<Vec<WorkspaceChange>> {
    ...
    for file_path in &files {
        ...
        // Skip .mic directory
        if relative.starts_with(".mic") || relative.starts_with(".mic/") {
            continue;
        }
...
fn scan_dir(
    ...
    for entry in fs::read_dir(dir)? {
        ...
        if path.is_dir() {
            // Skip .mic and .git directories
            let name = path.file_name().and_then(|n| n.to_str()).unwrap_or("");
            if name == ".mic" || name == ".git" || name == "node_modules" {
                continue;
            }
...
mod tests {
    ...
    fn test_ignore_patterns() {
        ...
        assert!(ignore.is_ignored(".mic"));
        assert!(ignore.is_ignored(".mic/session.bin"));
    ...
    fn test_scan_ignores_mic_dir() {
        let dir = tempdir().unwrap();
        fs::write(dir.path().join("file.txt"), "content").unwrap();
        fs::create_dir(dir.path().join(".mic")).unwrap();
        fs::write(dir.path().join(".mic/session.bin"), "session").unwrap();
