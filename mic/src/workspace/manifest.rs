...
use crate::workspace::{ensure_mic_dir, mic_dir, MANIFEST_FILE, MIC_DIR};
...
impl Manifest {
    ...
    pub fn find_and_load() -> Result<Self> {
        ...
        loop {
            let manifest_path = current.join(MIC_DIR).join(MANIFEST_FILE);
...
impl WorkspaceManifest {
    ...
    pub fn load() -> Result<Option<Self>> {
        let path = mic_dir()?.join(MANIFEST_FILE);
        Self::load_from(&path)
    }
