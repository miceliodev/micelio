...
use crate::workspace::{ensure_mic_dir, ensure_overlay_dir, is_safe_path, mic_dir, clear_overlay, OVERLAY_DIR, SESSION_FILE};
use base64::Engine;
        ...
        if let Some(ref bloom_data) = state.bloom_data {
            if let Ok(bloom_bytes) = base64::engine::general_purpose::STANDARD.decode(bloom_data) {
                if let Some(mut bloom) = Bloom::deserialize(&bloom_bytes) {
                    bloom.add(path);
                    state.bloom_data = Some(
                        base64::engine::general_purpose::STANDARD.encode(bloom.serialize()),
                    );
                }
            }
        }
...
fn overlay_file_path(path: &str) -> Result<PathBuf> {
    Ok(mic_dir()?.join(OVERLAY_DIR).join(path))
}
