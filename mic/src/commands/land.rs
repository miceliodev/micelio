...
use crate::cli::{looks_like_project_ref, parse_project_ref, LandCommand, SessionCommand, SessionSubcommand};
...
/// - In workspace: `mic land "goal"` - project inferred from workspace
/// - Outside workspace: `mic land org/project "goal"` - project explicit
pub async fn run(cmd: LandCommand) -> Result<()> {
    ...
    Session::start(&org, &project, &goal)?;
        ...
        None => {
            ...
            if looks_like_project_ref(first) {
                return Err(MicError::Other(
                    "Missing goal. Usage: mic land <org/project> \"<goal>\"".to_string(),
                ));
            }
            ...
            let manifest = Manifest::find_and_load().map_err(|_| {
                MicError::NotInWorkspace(
                    "Not in a workspace. Either:\n  \
                     ...
                     2. Specify the project: mic land <org/project> \"<goal>\""
                        .to_string(),
                )
            })?;
