...
use crate::workspace::{WorkspaceManifest, session::Session, collect_changes, ChangeType};
...
pub async fn run() -> Result<()> {
    ...
    println!("{}", format!("On project {}/{}", manifest.account, manifest.project).bold());
    ...
    if let Some(state) = Session::load()? {
        ...
        let staged_paths: std::collections::HashSet<_> = state.files.iter().map(|f| &f.path).collect();
        ...
        if !unstaged.is_empty() {
            ...
            println!();
            println!("Use 'mic write <path>' to stage changes.");
        }
