defmodule Micelio.Plans do
  ...
  alias Micelio.Forges.PullRequests
  alias Micelio.LLM
  ...
  def start_agentic_session(%Plan{} = plan, opts) do
    ...
    with :ok <- authorize_session_start(plan, user),
         {:ok, plan} <- ensure_forge_pull_request(plan, user),
  ...
  def ensure_plan_forge_pull_request(%Plan{} = plan, user) do
    with :ok <- authorize_session_start(plan, user) do
      ensure_forge_pull_request(plan, user)
    end
  end
  ...
  defp ensure_forge_pull_request(%Plan{} = plan, %{} = user) do
    plan = Repo.preload(plan, :repository)

    case plan.repository do
      %Repository{forge_provider: nil} ->
        {:ok, plan}

      %Repository{} = repository ->
        branch_name = plan.forge_branch_name || "micelio/#{plan.id}"

        if plan.forge_pr_url && plan.forge_pr_number do
          {:ok, plan}
        else
          with {:ok, pr} <-
                 PullRequests.ensure_draft_pr(
                   repository,
                   user,
                   branch_name: branch_name,
                   title: forge_pr_title(plan),
                   description: forge_pr_description(plan)
                 ) do
            plan
            |> Plan.forge_pr_changeset(%{
              forge_branch_name: branch_name,
              forge_pr_provider: pr.provider,
              forge_pr_number: normalize_pr_number(pr.number),
              forge_pr_url: pr.url,
              forge_pr_state: pr.state,
              forge_pr_draft: pr.draft,
              forge_pr_synced_at: DateTime.utc_now() |> DateTime.truncate(:second),
              forge_pr_metadata: pr.metadata || %{}
            })
            |> Repo.update()
          end
        end
  ...
  defp forge_pr_title(%Plan{} = plan) do
    plan.title || "Micelio session #{String.slice(plan.id, 0, 8)}"
  end

  defp forge_pr_description(%Plan{} = plan) do
    parts =
      [
        plan.description,
        plan.prompt
    ...
    case parts do
      [] ->
        "Generated by Micelio session #{plan.id}"
  ...
  defp normalize_pr_number(number) when is_integer(number), do: number
  ...
  defp normalize_pr_number(_), do: nil
